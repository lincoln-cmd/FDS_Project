...
def upload_form(request):
    if request.method == 'POST':
        ...
        try:
            # MySQL 연결을 통해 데이터베이스 생성
            with connection.cursor() as cursor:
                cursor.execute(create_db_sql)

            # 사용자 데이터베이스에 연결하여 작업 진행
            with connections[db_name].cursor() as cursor:
                ...
                # IsolationForest 모델로 이상 거래 예측
                model = load_model()  # IsolationForest 모델 로드 함수

                # CSV 파일의 각 행을 처리하여 데이터 저장
            ...
            print(f"Redirecting to: {kibana_url}")
...
def create_kibana_index_pattern(index_name):
    kibana_url = "http://211.188.51.137:5601/api/saved_objects/index-pattern"
    headers = {
        "kbn-xsrf": "true",
    ...
    index_pattern_data = {
        "attributes": {
            "title": index_name,
            "timeFieldName": "@timestamp"  # 혹은 사용자가 지정한 타임 필드
        }
    }

    response = requests.post(kibana_url, headers=headers, json=index_pattern_data)
